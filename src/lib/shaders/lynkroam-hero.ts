...
float routeLine(vec2 point, float offset, float pace) {
  ...
  path += sin(point.x * 5.1 - u_time * 0.07 + offset) * 0.045;
...
void main() {
  ...
  vec2 movingUv = uv;
  movingUv.x += sin(u_time * 0.14) * 0.075;
  ...
  float routes = routeLine(movingUv, 0.0, 0.17);
  routes += routeLine(movingUv + vec2(0.18, 0.38), 1.8, -0.11) * 0.72;
  routes += routeLine(movingUv - vec2(0.12, 0.42), 3.7, 0.09) * 0.55;
